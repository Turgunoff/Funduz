      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
        ...
        >
          <div class="relative w-full aspect-[4/3] bg-gray-100">
            <img :src="project.image" alt="Project" class="w-full h-full object-cover"/>
            ...
            <div class="mt-auto space-y-4">
              <div v-if="project.status === 'active'">
                <div class="flex justify-between items-end mb-2">
                  <div class="text-[15px] font-black text-gray-900">{{ project.raised }} so'm</div>
                  <div class="text-[13px] font-black text-[#1a946b]">{{ project.progress }}%</div>
                </div>
                <div class="w-full h-2 bg-gray-100 rounded-full overflow-hidden">
                  <div class="h-full bg-[#1a946b]" :style="{ width: project.progress + '%' }"></div>
                </div>
              </div>
              <div class="grid grid-cols-2 gap-3 pt-4 border-t border-gray-50">
                <button class="w-full py-2.5 bg-gray-50 text-gray-700 font-bold text-[13px] rounded-xl hover:bg-gray-100 transition-colors">
                  Измeнить
                </button>
                <button class="w-full py-2.5 bg-[#f0f9f6] text-[#1a946b] font-bold text-[13px] rounded-xl hover:bg-[#1a946b] hover:text-white transition-colors">
                  Статистика
                </button>
              </div>
            </div>
...
// Mock projects string. You can toggle this to an empty array to see the empty state.
const myProjects = ref([
  {
    id: 1,
    title: 'Инновационный образовательный центр для детей',
  ...
  {
    id: 2,
    title: 'Мобильное приложение для сортировки мусора',
